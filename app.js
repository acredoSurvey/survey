...
(function () {
  ...
  function polaroidConfig() {
    ...
    return {
      ...
      spawnEveryMs: p.spawnEveryMs != null ? p.spawnEveryMs : 920,
      fallDurationMs: p.fallDurationMs != null ? p.fallDurationMs : 15000,
      maxOnScreen: p.maxOnScreen != null ? p.maxOnScreen : 9,
      maxInTopThird: p.maxInTopThird != null ? p.maxInTopThird : 4,
      minLaneSepPct: p.minLaneSepPct != null ? p.minLaneSepPct : 11,
    };
  }
  ...
  function pickPolaroidLane(activeMeta, minSep) {
    const sep = minSep != null ? minSep : 12;
    const occupiedRecent = new Set();
    const activeLefts = [];
    activeMeta.forEach((m) => {
      activeLefts.push(m.left);
      if (m.lane != null && m.ageMs < 1100) occupiedRecent.add(m.lane);
    });

    const sides = [6, 14, 22, 30, 70, 78, 86, 94];
    const noCenter = [6, 14, 22, 30, 38, 62, 70, 78, 86, 94];
    const source = preferSideLanes() ? sides : noCenter;
    const emptyLanes = source.filter((lane) => !occupiedRecent.has(lane));
    const pool = (emptyLanes.length ? emptyLanes : source.slice())
      .slice()
    ...
    for (let i = 0; i < source.length; i++) {
      ...
      if (nearest > fallbackScore) {
        ...
        fallback = { left: left, lane: lane };
      }
    }
    return fallback || { left: 22, lane: 22 };
  }
  ...
  function spawnOnePolaroid(cfg, opts) {
    ...
    if (!isStatic) {
      ...
      const cap = Math.min(
        cfg.maxOnScreen != null ? cfg.maxOnScreen : 7,
        unique
      );
    ...
    const pick = pickPolaroidLane(
      ...
      cfg.minLaneSepPct != null ? cfg.minLaneSepPct : 12
    );
    ...
    const size = wide ? randBetween(72, 86) : randBetween(56, 68);
  ...
  function scheduleNextPolaroid(cfg, waitMs) {
    ...
    polaroidSpawnTimer = window.setTimeout(() => {
      ...
      const every = cfg.spawnEveryMs != null ? cfg.spawnEveryMs : 920;
  ...
  function startPolaroidRain() {
    ...
    const kickOff = () => {
      if (!polaroidActive) return;
      spawnOnePolaroid(cfg);
      scheduleNextPolaroid(cfg, cfg.spawnEveryMs != null ? cfg.spawnEveryMs : 920);
    };

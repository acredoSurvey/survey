...
const CONFIG = {
  ...
  /* 프러포즈 문구가 시작될 때 재생 */
  proposalMusic: "love_wins_all.mp3",
  ...
  /* 랜딩 참여 혜택 문구 */
  landingMeta: "약 2–3분 · 10문항\n설문 기간: 2026년 9월 14일 ~ 9월 19일",
  landingReward: "설문 완료 시 스타벅스 아메리카노 Tall을 증정해드립니다.",
  landingRewardBoth:
    "신랑·신부 모두 설문에 참여해 주셔야 기프티콘이 지급됩니다.",
  ...
  questions: [
    ...
    },
    {
      ...
      options: ["지인 추천", "SNS(페이스북, 인스타 등)", "검색 (네이버·구글)", "웨딩 박람회/매장 방문", "블로그·후기", "기타"],
    },
    {
      id: "whyChose",
      type: "choice",
...
(function () {
  ...
  let revealTimers = [];
  let proposalAudio = null;
  let proposalAudioUnlocked = false;
  let proposalMusicFade = null;

  function proposalMusicSrc() {
    return (CONFIG.proposalMusic || "").trim();
  }

  function getProposalAudio() {
    const src = proposalMusicSrc();
    if (!src) return null;
    if (proposalAudio) return proposalAudio;
    proposalAudio = new Audio(src);
    proposalAudio.preload = "auto";
    proposalAudio.loop = false;
    proposalAudio.playsInline = true;
    proposalAudio.setAttribute("playsinline", "");
    proposalAudio.volume = 0;
    return proposalAudio;
  }

  function unlockProposalAudio() {
    const audio = getProposalAudio();
    if (!audio || proposalAudioUnlocked) return;
    audio.muted = true;
    audio.volume = 0;
    const p = audio.play();
  ...
  function startProposalMusic() {
    const audio = getProposalAudio();
    if (!audio) return;
    if (!audio.paused && audio.currentTime > 0.15 && !audio.muted) return;
    audio.muted = false;
    audio.volume = 0;
    try {
      audio.currentTime = 0;
    ...
    const p = audio.play();
    function fadeIn() {
      if (proposalMusicFade) window.clearInterval(proposalMusicFade);
      const target = 0.5;
      proposalMusicFade = window.setInterval(function () {
        const next = Math.min(target, (audio.volume || 0) + 0.03);
        audio.volume = next;
        if (next >= target) {
          window.clearInterval(proposalMusicFade);
          proposalMusicFade = null;
        }
      }, 70);
    }
    if (p && p.then) {
  ...
  function revealTiming() {
    ...
    return {
      ...
      askBefore: t.askBefore != null ? t.askBefore : 720,
      askHold: t.askHold != null ? t.askHold : 600,
      actions: t.actions != null ? t.actions : 200,
      screenDwell: t.screenDwell != null ? t.screenDwell : 7500,
      askDwell: t.askDwell != null ? t.askDwell : 2600,
      exitFade: t.exitFade != null ? t.exitFade : 900,
      enterFade: t.enterFade != null ? t.enterFade : 2100,
      lineStagger: t.lineStagger != null ? t.lineStagger : 400,
      askEnterFade: t.askEnterFade != null ? t.askEnterFade : 1400,
      askLineStagger: t.askLineStagger != null ? t.askLineStagger : 720,
      gapBetweenMs: t.gapBetweenMs != null ? t.gapBetweenMs : 700,
    };
  }
  ...
  function polaroidConfig() {
    ...
    return {
      ...
      spawnEveryMs: p.spawnEveryMs != null ? p.spawnEveryMs : 780,
      fallDurationMs: p.fallDurationMs != null ? p.fallDurationMs : 15000,
      maxOnScreen: p.maxOnScreen != null ? p.maxOnScreen : 10,
      maxInTopThird: p.maxInTopThird != null ? p.maxInTopThird : 3,
      minLaneSepPct: p.minLaneSepPct != null ? p.minLaneSepPct : 14,
    };
  }
  ...
  function clearPolaroidTimers() {
    ...
    if (polaroidSpawnTimer != null) {
      window.clearTimeout(polaroidSpawnTimer);
  ...
  function getActivePolaroidSrcs() {
    if (!els.polaroidRain) return new Set();
    return new Set(
      Array.from(els.polaroidRain.querySelectorAll(".polaroid"))
        .map((node) => node.dataset.src)
        .filter(Boolean)
    );
  }

  function preloadPolaroidSrc(src) {
    if (!src) return;
    const img = new Image();
    img.decoding = "async";
    img.src = src;
  ...
  function refillPolaroidDeck(images, excludeSrcs) {
    const exclude = excludeSrcs || new Set();
    polaroidDeck = shuffleInPlace(images.filter((src) => !exclude.has(src)));
  }

  function nextPolaroidSrc(images) {
    if (!images.length) return null;
    const onScreen = getActivePolaroidSrcs();
    polaroidDeck = polaroidDeck.filter((src) => !onScreen.has(src));
    if (!polaroidDeck.length) refillPolaroidDeck(images, onScreen);
    if (!polaroidDeck.length) return null;
    const src = polaroidDeck.pop();
    if (polaroidDeck.length) preloadPolaroidSrc(polaroidDeck[polaroidDeck.length - 1]);
    return src;
  }
  ...
  function preferSideLanes() {
    if (!els.proposalStage) return false;
    if (els.proposalStage.classList.contains("is-ask-final")) return true;
    return !!(
      els.proposalFocus &&
      els.proposalFocus.querySelector(".proposal__line.is-shown")
    );
  }

  function polaroidLanes() {
    const wide =
      window.matchMedia && window.matchMedia("(min-width: 700px)").matches;
    if (preferSideLanes()) {
      return wide ? [4, 16, 28, 72, 84, 96] : [4, 18, 32, 68, 82, 96];
    }
    return wide ? [4, 16, 28, 50, 72, 84, 96] : [4, 18, 36, 64, 82, 96];
  }

  function pickPolaroidLane(minSep, sizePx) {
    const rain = els.polaroidRain
      ? els.polaroidRain.getBoundingClientRect()
      : null;
    if (!rain || !rain.width) return null;

    const sizePct = (sizePx / rain.width) * 100;
    const sep = Math.max(minSep != null ? minSep : 14, sizePct * 0.7);
    const heightPx = sizePx * 1.32;
    const pad = 16;
    const source = polaroidLanes();
    const meta = getActivePolaroidsMeta();
    const spawnTop = rain.top - rain.height * 0.1;
    const spawnBottom = spawnTop + heightPx + pad;
    const pool = source.slice().sort(() => Math.random() - 0.5);

    let best = null;
    let bestScore = -1;
    for (let i = 0; i < pool.length; i++) {
      const lane = pool[i];
      const left = Math.max(2, Math.min(86, lane + randBetween(-0.8, 0.8)));
      const spawnLeft = rain.left + (left / 100) * rain.width;
      const spawnRight = spawnLeft + sizePx;
      ...
      for (let j = 0; j < meta.length; j++) {
        ...
        const clearX = spawnRight + pad <= r.left || r.right + pad <= spawnLeft;
        if (!clearX && r.top < spawnBottom + 28) {
          blocked = true;
          break;
        }
        if (r.top < rain.top + rain.height * 0.38) {
          nearestHigh = Math.min(nearestHigh, Math.abs(m.left - left));
        }
      }
      if (blocked) continue;
      if (nearestHigh < sep) continue;
      if (nearestHigh > bestScore) {
        bestScore = nearestHigh;
  ...
  function spawnOnePolaroid(cfg, opts) {
    if (!els.polaroidRain || !cfg.images.length) return false;

    const isStatic = !!(opts && opts.static);
    if (!isStatic) {
      const unique = cfg.images.length;
      const cap = Math.min(
        cfg.maxOnScreen != null ? cfg.maxOnScreen : 10,
        unique
      );
      if (els.polaroidRain.querySelectorAll(".polaroid").length >= cap) {
        return false;
      }
      const topCap = cfg.maxInTopThird != null ? cfg.maxInTopThird : 3;
      if (topCap < 90 && countPolaroidsInTopThird() >= topCap) {
        return false;
      }
    }
    ...
    const size = wide ? randBetween(86, 104) : randBetween(68, 82);

    const pick = pickPolaroidLane(
      cfg.minLaneSepPct != null ? cfg.minLaneSepPct : 14,
      size
    );
    ...
    const rot = randBetween(-5, 5);
    const opacity = randBetween(0.82, 0.92);
    const duration = cfg.fallDurationMs != null ? cfg.fallDurationMs : 15000;
    ...
    if (opts && opts.static) {
      ...
      el.style.opacity = String(Math.min(0.94, opacity));
    } else {
  ...
  function scheduleNextPolaroid(cfg, waitMs) {
    if (polaroidSpawnTimer != null) {
      window.clearTimeout(polaroidSpawnTimer);
      polaroidSpawnTimer = null;
    }
    polaroidSpawnTimer = window.setTimeout(() => {
      polaroidSpawnTimer = null;
      if (!polaroidActive) return;
      spawnOnePolaroid(cfg);
      const every = cfg.spawnEveryMs != null ? cfg.spawnEveryMs : 780;
      scheduleNextPolaroid(cfg, every);
    }, waitMs);
  }
  ...
  function startPolaroidRain() {
    ...
    polaroidDeck.slice(-3).forEach(preloadPolaroidSrc);
    polaroidActive = true;
    ...
    const kickOff = () => {
      if (!polaroidActive) return;
      spawnOnePolaroid(cfg);
      scheduleNextPolaroid(cfg, cfg.spawnEveryMs != null ? cfg.spawnEveryMs : 780);
    };
  ...
  function softenPolaroidRainForCelebration() {
    ...
    if (prefersReducedMotion()) return;
    const base = polaroidConfig();
    const cfg = Object.assign({}, base, {
      spawnEveryMs: Math.max(base.spawnEveryMs * 1.4, 1600),
      maxOnScreen: Math.min(base.maxOnScreen, 8),
    });
    scheduleNextPolaroid(cfg, cfg.spawnEveryMs);
  }
  ...
  function fadeOutFocus(done) {
    ...
    els.proposalFocus
      .querySelectorAll(".proposal__focus-stack")
    ...
    if (isAsk) {
      ...
      if (askImg) {
        ...
        const imgDelay = reduced
          ? "0ms"
          : (parts.length - 1) * stagger + Math.round(enterMs * 0.55) + "ms";
  ...
  function screenEnterDwell(lineCount) {
    ...
    const stagger = t.lineStagger != null ? t.lineStagger : 320;
    const enterMs = t.enterFade != null ? t.enterFade : 2100;
    const readPad = t.screenDwell != null ? t.screenDwell : 7500;
    const extraRead = Math.max(0, n - 2) * 400;
    return (n - 1) * stagger + enterMs + readPad + extraRead;
  }
  ...
  function runStagedReveal() {
      ...
      if (isAsk) {
        ...
        fadeOutFocus(() => {
          after(t.askBefore, () => {
            ...
            const enterMs =
              t.askEnterFade != null ? t.askEnterFade : 1400;
            const askPad = t.askDwell != null ? t.askDwell : t.askHold;
            const stagger =
              t.askLineStagger != null ? t.askLineStagger : 720;
            const n = (screen.lines && screen.lines.length) || 1;
            const artExtra =
              (CONFIG.proposalAskImage || "").trim()
                ? (n - 1) * stagger + Math.round(enterMs * 0.55) + 400
    ...
    after(Math.max(0, cursor - 2000), startProposalMusic);
  ...
  const metaEl = document.getElementById("landing-meta");
  if (metaEl && CONFIG.landingMeta) {
    metaEl.innerHTML = String(CONFIG.landingMeta).replace(/\n/g, "<br />");
  }
  ...
  if (rewardEl && CONFIG.landingReward) {
    ...
    const bothText = (CONFIG.landingRewardBoth || "").trim();
    if (bothText) {
      const both = document.createElement("span");
      both.className = "reward__note";
      both.id = "landing-reward-both";
      both.textContent = bothText;
      rewardEl.appendChild(both);
    }

...
(function () {
  ...
  function polaroidConfig() {
    ...
    return {
      ...
      maxOnScreen: p.maxOnScreen != null ? p.maxOnScreen : 7,
      maxInTopThird: p.maxInTopThird != null ? p.maxInTopThird : 99,
      minLaneSepPct: p.minLaneSepPct != null ? p.minLaneSepPct : 12,
    };
  }
  ...
  function pickPolaroidLane(activeMeta, minSep) {
    ...
    activeMeta.forEach((m) => {
      ...
      if (m.lane != null && m.ageMs < 900) occupiedRecent.add(m.lane);
    });
    ...
    for (let i = 0; i < pool.length; i++) {
      const lane = pool[i];
      const left = Math.max(3, Math.min(88, lane + randBetween(-2, 2)));
    ...
    return fallback || { left: 48, lane: 46 };
  }
  ...
  function spawnOnePolaroid(cfg, opts) {
    ...
    if (!isStatic) {
      const cap = cfg.maxOnScreen != null ? cfg.maxOnScreen : 7;
      /* 중간에 끊지 않음 — 아래 밖으로 페이드아웃될 때까지 유지 */
    ...
    /* 일자 낙하: 흔들림·회전 변화 없음 (약간의 고정 기울기만) */
    const rot = randBetween(-6, 6);
    const rx = 0;
    const ry = 0;
    const wide =
      window.matchMedia && window.matchMedia("(min-width: 600px)").matches;
    const size = wide ? randBetween(70, 92) : randBetween(54, 72);
    const opacity = randBetween(0.42, 0.58);
    const sway = 0;
  ...
  function startPolaroidRain() {
    ...
    /* 비처럼: 고정 간격으로 딱 1장만. 실패해도 몰아서 재시도하지 않음 */
    const kickOff = () => {
